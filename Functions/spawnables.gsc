...
spawn_merryGoRound()
{
    ...
    sizes       = strTok("0;43;75;107;138;170",";");
    sizesSeats  = strTok("93;139;93;139;93;139;93;139;93;139",";");
    sizesLights = strTok("104;138;104;138;104;138;104;138;104;138",";");
    ...
    {
        level.merryGoRound[level.merryGoRound.size] = modelSpawner(level.merryGoRoundMove.origin + (cos(i* 360 / (6*e)) * int(sizes[e]), sin(i* 360 / (6*e)) * int(sizes[e]), -20 + 180*z), "com_plasticcase_friendly", (0, (i* 360 / (6*e)) + 90, 0), time, level.airDropCrateCollision );
        level.merryGoRound[level.merryGoRound.size -1] linkTo(level.merryGoRoundMove);
    }
    
    for(e = 0; e < 10; e++) 
    {
        level.merryGoRoundLights[e] = modelSpawner(level.merryGoRoundMove.origin + (cos(e*36)*int(sizesLights[e]),sin(e*36)*int(sizesLights[e]), 148), "tag_origin", (0,(e*36)+0,0), time );
        level.merryGoRoundLights[e+10] = modelSpawner(level.merryGoRoundMove.origin + (cos(e*36)*168,sin(e*36)*168, 148), "tag_origin", (0,(e*36)+0,0), time);
        wait .05;
        ...
        level.merryGoRoundSeats[level.merryGoRoundSeats.size] = modelSpawner(level.merryGoRoundMove.origin + (cos(e*36)*int(sizesSeats[e]),sin(e*36)*int(sizesSeats[e]), 12), "com_plasticcase_friendly", (0,(e*36)+0,0), time );
        level.merryGoRoundSeatTags[level.merryGoRoundSeatTags.size] = modelSpawner(level.merryGoRoundMove.origin + (0,0,12), "tag_origin", (0,(e*36)+0,0), time);
...
do_ferrisWheel()
{
    ...
    foreach(tags in level.FerrisSeats)
        tags thread monitorSeats( "Ferris Wheel", level.FerrisSeats );
}
...
do_centrox()
{
    ...
    foreach( seat in level.centrox_seats )
        seat thread monitorSeats( "Centrox", level.centrox_seats );
...
//SEAT MONITORING
monitorSeats( ride, seats_array ) //WILL RUN ON EACH SEAT
{
    level endon("game_ended");
    self endon("death");
    ...
    while( IsDefined( self ) )
    {
        foreach( player in level.players )
        {
            if( Distance( player.origin, self.origin ) < 170 && !isDefined(player.in_seat) && !seat_taken )
            {
                player thread doHintString( "Press [{+usereload}] to ride the " + ride + ".", self.origin, 999 );
                ...
                if( player UseButtonPressed() )
                {
                    ...
                    get_seat = getClosest( player.origin, seats_array );
                    if( ride == "Ferris Wheel" )
                    {
                        seat = modelSpawner( get_seat.origin - anglesToRight(self.angles) * 22, "script_origin", (0,90,0) );
                        seat thread seatAngleFix( get_seat );
                        player PlayerLinkToDelta( seat );
                    }
                    else if( ride == "Centrox" || ride == "Fireball" )
                        player PlayerLinkToAbsolute( get_seat, "tag_origin" ); 
                    else    
                        player playerLinkTo( get_seat );
                    
                    level notify("player_is_riding");
                    ...
                    player thread doHintString( "Press [{+melee}] to exit the " + ride + ".", self.origin, 999 );    
                    while( IsDefined( player ) )
                    {
                        if( player MeleeButtonPressed() || isDefined( player.kicked_from_ride ) )
                        {
                            if(!isDefined(player.godmode))
            ...
            else if( isDefined( player.hintString ) )
                player.hintString destroy();
        }
        wait .05;
    }
...
trade_weap_table()
{
    if(!IsDefined( self.trade_table ))
    {
        ...
        weapon_table     = modelSpawner( self.origin + (0,0,12) + AnglesToForward( self.angles ) * 70, "com_plasticcase_enemy", self.angles + (0,270,0)); 
        weapon_table thread trade_weap_monitor( self );
    }
    else 
    {
        self.trade_table = undefined;
    }
    self setColour(self.trade_table);
}

trade_weap_monitor( pers )
{
    table_status = "place";
    while(isDefined(pers.trade_table))
    {
        foreach( player in level.players )
        {
...
spawn_fireball()
{
    ...
    for(e=0;e<4;e++)
    {
        ...
        level.fireball.cart_tags[e] thread monitorSeats( "Fireball", level.fireball.cart_tags );
    }
...
spawn_vertigo()
{
    ...
    foreach(seat in level.vertigo.seat)  
    {
        ...
        seat thread monitorSeats( "Vertigo", level.vertigo.seat );
    }
