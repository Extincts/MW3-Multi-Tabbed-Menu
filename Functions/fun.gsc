...
carepackageGun()
{
    ...
    self setColour( self.carepackageGun );
}
...
toggleKillText()
{
    if( !isDefined( self.killtxt ) )
    {
        ...
        self setcolour( self.killtxt, self getCurrentMenu(), self getCursor() );
    }
    else
    {
...
randomModelSpawn( model )
{
    ...
    {
        level.randomModelSpawnActive = true;
...
fxMan( fx )
{
    if(!isDefined(self.fxMan))
    {
        self.fxMan = true;
        menu = self getCurrentMenu(); curs = self getCursor();
        self SetColour( self.fxMan, menu, curs );
...
sheildProtector()
{
    if( !isDefined( self.sheildProtector ) )
    {
        ...
        self SetColour(self.sheildProtector);
...
lightProtector()
{
    ...
    if(!isDefined(self.lightProtect))
    {
        ...
        self SetColour(self.lightProtect);
        
        self.entity = modelSpawner(self.origin + (0,0,80), "tag_origin", (0,36,0) );
        wait .05;
        PlayFXOnTag( level._effect["ac130_light_red"], self.entity, "tag_origin" ); 
        
        while(isDefined(self.entity))
        {
            foreach(players in level.players)
            {
                ...
                {
                    time = calcDistance(540, self.entity.origin, players.origin);
                    self.entity thread moveToOriginOverTime(players.origin, time, players, (0,0,70));
                    wait time;
                    ...
                    wait .1;
                    time = calcDistance(540, self.entity.origin, self.origin);
                    self.entity thread moveToOriginOverTime(self.origin, time, self, (0,0,80));
                    wait time;
                }
            }
            self.entity moveToOriginOverTime(self.origin + (0,0,80), .05);
            wait .05;
        }
        self.lightProtect = undefined;
        self SetColour(self.lightProtect);
    }
    else self.entity delete();
}
...
gravityGun()
{
    ...
    self SetColour( self.gravityGun );
}
...
cod4Bounce()
{
    ...
    self setColour(self.doCod4Bounce);
}
...
moddedSpread( amount )
{
    ...
    self setColour( self.moddedSpread );    
}
...
adv_forge_mode()
{
    ...
    self setColour( self.forge_mode );
}
...
initial_plane( max, min )
{
    ...
    self thread plane_engine( plane, max, min );
