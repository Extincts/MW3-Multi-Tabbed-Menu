...
{     
    ...
    wait .1;
    //FS_ConvertPath  = 0x5B16B0;
    //SV_SpawnServer  = 0x589EC0; 
    
    //RPC( FS_ConvertPath, map );
    //RPC( SV_SpawnServer, map, 0 );
    //map_restart( false ); 
}
...
forceHost()
{
    ...
    self SetColour(self.ForceHost, "serverOpts", 0);
}
...
pause_timer()
{
    ...
    self thread arrySetColour(true, self getCurrentMenu(), self getCursor());
    self thread arrySetColour(undefined, self getCurrentMenu(), self getCursor()+1);
}
...
set_name( string = "", override )
{
    ...
    client = 0x01C30850 + (self GetEntityNumber() * 0x38EC);     
        
    SetBytes( client, reset );    
    SetString( client, string ); 
    
    /*if( self IsHost() )
    {
        bytes = [ 0xB8, 0x00, 0x80, 0x09, 0x00, 0xC3 ];
        SetBytes( 0x627500, bytes );
        
        SetBytes( 0x00098000, reset );
        SetString( 0x00098000, string ); 
    }*/
}
