toggleAimbot( azza )
{
    ...
    self setColour(self.aimbotT, "aimbotSystem", 0);    
} 
...
aimbotChecks( check, opt )
{
    ...
    self SetColour(self.aimbot[check], "aimbotSystem", opt);    
...
fakeShoot( target ) 
{
    ...
    //RPC(0x0057A6D0, (0x194B9D0 + (self getEntityNumber() * 0x280)), 0);
...
resetAllPresets( all )
{
    ...
    self SetColour(self.azzaPreset, "aimbotPresets", 0);
    self SetColour(self.unfairPreset, "aimbotPresets", 1);
    self SetColour(self.sniperPreset, "aimbotPresets", 2);
