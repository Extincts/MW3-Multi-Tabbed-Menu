...
cinematicTele()
{
    ...
    self SetColour(self.cinematicTele);
}
...
{
    if(team == "Closest") 
        return self setOrigin(level.players[self returnClosest()].origin);
...
advancedSaveLoad()
{
    ...
    if( !isDefined( self.adSaveLoad ) && !isDefined( self.saveLoad ))
    {
        ...
        self SetColour(self.adSaveLoad);
...
allToMe( team, crosshair )
{
    ...
    {
        if( team == "Closest" )
            player = level.players[self returnClosest()];
...
returnClosest()
{
    foreach(player in level.players)
    {
        if(Closer( self getTagOrigin("j_head"), player getTagOrigin("j_head"), level.players[final] getTagOrigin("j_head") ) && player != self)
            final = player getEntityNumber();
    }
    return final;
}
